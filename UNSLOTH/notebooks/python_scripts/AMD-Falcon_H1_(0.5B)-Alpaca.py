...
# get_ipython().system('uv pip install --system -qqq --no-deps accelerate peft "trl==0.22.2" "causal-conv1d==1.5.0.post8"')
# get_ipython().system('uv pip install --system -qqq --no-build-isolation "mamba-ssm==2.2.4"')
# 
# 
# # ### Unsloth
...
model.save_pretrained("falcon_h1_lora")  # Local saving
tokenizer.save_pretrained("falcon_h1_lora")
# model.push_to_hub("your_name/falcon_h1_lora", token = "YOUR_HF_TOKEN") # Online saving
# tokenizer.push_to_hub("your_name/falcon_h1_lora", token = "YOUR_HF_TOKEN") # Online saving
...
if False:
    from unsloth import FastLanguageModel
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name = "falcon_h1_lora", # YOUR MODEL YOU USED FOR TRAINING
        max_seq_length = max_seq_length,
        dtype = dtype,
        load_in_4bit = load_in_4bit,
    )
...
if False:
    ...
    model = AutoPeftModelForCausalLM.from_pretrained(
        "falcon_h1_lora", # YOUR MODEL YOU USED FOR TRAINING
        load_in_4bit = load_in_4bit,
    )
...
if False:
    model.push_to_hub_gguf(
        "HF_USERNAME/falcon_h1_finetune", # Change hf to your username!
        tokenizer,
